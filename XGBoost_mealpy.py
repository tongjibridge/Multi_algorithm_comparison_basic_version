...
import pickle  # 导入pickle库，用于模型的保存和加载
...
# 解决 xgb 在 GridSearchCV 中可能出现的 verbosity 警告
...
class XGBoostProblem(Problem):
    def __init__(
        ...
        **kwargs,
        ...
        self.non_standardize_features = non_standardize_features
    ...
    def obj_func(self, x):
        ...
        x_decode = self.decode_solution(x)
        error = 0
        count = 0
        kf = KFold(n_splits=5, shuffle=True, random_state=42)

        for train_index, test_index in kf.split(self.y):
            x_train, x_test = self.x.iloc[train_index], self.x.iloc[test_index]
            y_train, y_test = self.y.iloc[train_index], self.y.iloc[test_index]

            # 使用one-hot编码时删除
            x_train, encoder, categorical_feature_names = encode_database(
                x_train, y_train, self.categorical_columns
            )
            ...
            X_train_scaled_df, X_test_scaled_df = data_norm_get(
                ...
                y_train,
                y_test,
                non_standardize_features=self.non_standardize_features,
            )

            model = XGBRegressor(**x_decode, **xgb_param_grid)
...
bounds = [
    ...
    IntegerVar(lb=50, ub=300, name="n_estimators"),
    FloatVar(lb=0.3, ub=1.0, name="subsample"),
    IntegerVar(lb=3, ub=7, name="max_depth"),
]
problem = XGBoostProblem(
    bounds=bounds,
    minmax="max",
    x=x,
    y=y,
...
model_save_dir = r"./savemodel/xgb2meal/"  # 定义模型保存的目录
os.makedirs(model_save_dir, exist_ok=True)  # 创建目录，如果目录已存在则不报错
# --- 【xgb 修改】 ---
model_path = os.path.join(
    model_save_dir, "xgb_model_final.pkl"
)  # 定义模型的完整保存路径
scaler_path = os.path.join(model_save_dir, "scaler.pkl")  # 定义scaler的完整保存路径
joblib.dump(best_params, model_path)  # 将找到的最佳模型保存到文件
logger.debug(f"模型已保存至: {model_path}")  # 打印保存成功信息
best_params = joblib.load(model_path)  # 从文件加载模型参数
...
pickle.dump(best_model, open(os.path.join(model_save_dir, "xgb.pkl"), "wb"))
...
results_plot_save_dir = r"./result/xgb2meal/"  # 定义结果图保存的目录
...
test_df.columns = feature_names_from_df + ["y_test", "y_test_pred"]
with pd.ExcelWriter(os.path.join(results_plot_save_dir, "xgb_results.xlsx")) as writer:
    train_df.to_excel(writer, sheet_name="train", index=False)
    test_df.to_excel(writer, sheet_name="test", index=False)
...
train_path = os.path.join(
    results_plot_save_dir, "xgb_训练集精度_final.png"
)  # 训练集拟合图的保存路径
test_path = os.path.join(
    results_plot_save_dir, "xgb_验证集精度_final.png"
)  # 验证集拟合图的保存路径
# 调用函数绘制训练集的拟合图
plot_regression_fit2(
    y_train,
    ...
    train_mae,
    "Train Set",
    "xgb Model Performance (Train Set)",
    train_path,
)
# 调用函数绘制测试集的拟合图
plot_regression_fit2(
    ...
    test_mae,
    "Test Set",
    "xgb Model Performance (Test Set)",
    test_path,
)
...
# xgb feature_importances_ 默认基于'split'（特征在模型中被用作分裂节点的次数）
...
save_path_gbdt = os.path.join(
    results_plot_save_dir, "xgb_特征重要性组合图_final.png"
)  # 定义保存路径
# 调用函数绘制组合特征重要性图
plot_importance_combined(
    gbdt_importance_df,
...
save_path_perm = os.path.join(
    results_plot_save_dir, "xgb_特征重要性_Permutation_final.png"
)  # 定义保存路径
# 调用函数绘制组合特征重要性图（使用置换重要性数据）
plot_importance_combined(
    perm_importance_df,
    "特征重要性 (Permutation Importance for xgb)",
    save_path_perm,
...
train_res_path = os.path.join(
    results_plot_save_dir, "xgb_训练集残差分析图_final.png"
)  # 训练集残差图保存路径
test_res_path = os.path.join(
    results_plot_save_dir, "xgb_验证集残差分析图_final.png"
)  # 测试集残差图保存路径
# 调用函数绘制训练集残差图
plot_residuals_styled(
    train_residuals, y_train_pred, train_res_path, "xgb 训练集残差分析"
)
# 调用函数绘制测试集残差图
plot_residuals_styled(test_residuals, y_test_pred, test_res_path, "xgb 验证集残差分析")

# =================================================================================
# ============ 使用手动计算方法绘制 PDP 和 ICE 相关图 (适用于xgb) ============
# =================================================================================
logger.info(
...
# 定义PDP/ICE图的保存目录
# --- 【xgb 修改】 ---
pdp_ice_save_dir = os.path.join(results_plot_save_dir, "xgb_PDP_ICE_Plots_final")
...
for feature_name in top_features_pdp_names:
    ...
    try:
        ...
        # --- 【xgb 修改】 ---
        plt.savefig(
            os.path.join(pdp_ice_save_dir, f"xgb_Manual_PDP_ICE_{feature_name}.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close(fig)  # 关闭图表
...
if len(top_features_pdp_names) >= 2:  # 确保至少有两个特征可以进行组合
    ...
    for feat1, feat2 in combinations(top_features_pdp_names, 2):
        ...
        try:
            ...
            plt.savefig(
                os.path.join(
                    pdp_2way_save_dir, f"xgb_Manual_PDP_2D_{feat1}_{feat2}.png"
                ),
                dpi=300,
            )  # 保存
            ...
            # --- 【xgb 修改】 ---
            plt.savefig(
                os.path.join(pdp_3d_save_dir, f"xgb_Manual_PDP_3D_{feat1}_{feat2}.png"),
                dpi=300,
            )  # 保存
    ...
    for features_tuple in combinations(
        ...
        try:
            ...
            save_path = os.path.join(
                ...
                f"xgb_3D_Scatter_{features_tuple[0]}_{features_tuple[1]}_{features_tuple[2]}.png",
            )
            # 调用函数绘制3D散点图
            plot_3d_scatter_three_features(
        ...
        for feature_to_fix in features_tuple:
            try:
                ...
                save_path = os.path.join(
                    ...
                    f"xgb_3DPDP_{varying_feats[0]}_{varying_feats[1]}_Fix_{feature_to_fix}.png",
                )
                # 将固定的值设为该特征的中位数
...
# --- 【xgb 修改】 ---
logger.info("------------------------开始 SHAP 分析 (xgb)------------------------")
shap_save_dir = os.path.join(
    results_plot_save_dir, "xgb_SHAP_Plots_final"
)  # 定义SHAP图的保存目录
os.makedirs(shap_save_dir, exist_ok=True)  # 创建目录
# shap.TreeExplainer 同样适用于 xgb 模型
...
save_path_shap = os.path.join(
    shap_save_dir, "xgb_SHAP_特征重要性组合图_final.png"
)  # 定义保存路径
...
plt.savefig(
    os.path.join(shap_save_dir, "xgb_SHAP_summary_scatter.png"),
    dpi=300,
    bbox_inches="tight",
)  # 保存
plt.close()  # 关闭图表
...
for feature_name in top_features_pdp_names:
    ...
    plt.savefig(
        os.path.join(
            shap_dependence_save_dir, f"xgb_SHAP_dependence_{feature_name}.png"
        ),
        dpi=300,
        bbox_inches="tight",
    )  # 保存图表
...
plt.tight_layout()  # 调整布局
plt.savefig(
    os.path.join(shap_save_dir, "xgb_SHAP_waterfall_sample_0.png"),
    dpi=300,
    bbox_inches="tight",
)  # 保存
plt.close()  # 关闭图表
...
plt.tight_layout()  # 调整布局
plt.savefig(
    os.path.join(shap_save_dir, "cat_SHAP_waterfall_sample_0.png"),
    dpi=300,
    bbox_inches="tight",
)  # 保存
plt.close()  # 关闭图表
...
ale_save_dir = os.path.join(
    results_plot_save_dir, "xgb_ALE_Plots_final"
)  # 定义ALE图的保存目录
...
for i, feature_name in enumerate(top_features_ale_names):
    try:
        ...
        plt.tight_layout()  # 调整布局
        plt.savefig(
            os.path.join(ale_save_dir, f"xgb_ALE_1D_{feature_name}.png"),
            dpi=300,
            bbox_inches="tight",
        )  # 保存
        plt.close(fig)  # 关闭图表
...
if len(top_features_ale_names) >= 2:  # 确保至少有两个特征
    ...
    for feat1_name, feat2_name in combinations(top_features_ale_names, 2):
        try:
            ...
            plt.savefig(
                os.path.join(
                    ale_save_dir, f"xgb_ALE_2D_{feat1_name}_vs_{feat2_name}.png"
                ),
                dpi=300,
                bbox_inches="tight",
            )  # 保存图表
