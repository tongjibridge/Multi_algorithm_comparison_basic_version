...
model_save_dir = r"./savemodel/tabpfn2/"  # 定义模型保存的目录
os.makedirs(model_save_dir, exist_ok=True)  # 创建目录，如果目录已存在则不报错
# --- 【tabpfn 修改】 ---
model_path = os.path.join(
    model_save_dir, "tabpfn_model_final.tabpfn_fit"
)  # 定义模型的完整保存路径
...
if is_load == "n":
    best_model = TabPFNRegressor(memory_saving_mode="auto", n_jobs=-1, device="cuda")
    best_model.fit(X_train_scaled_df, y_train)
    model_loading.save_fitted_tabpfn_model(best_model, model_path)
else:
    best_model = model_loading.load_fitted_tabpfn_model(model_path, device="cuda")
...
results_plot_save_dir = r"./result/tabpfn2/"  # 定义结果图保存的目录
...
train_path = os.path.join(
    results_plot_save_dir, "tabpfn_训练集精度_final.png"
)  # 训练集拟合图的保存路径
test_path = os.path.join(
    results_plot_save_dir, "tabpfn_验证集精度_final.png"
)  # 验证集拟合图的保存路径
# 调用函数绘制训练集的拟合图
plot_regression_fit2(
    y_train,
    ...
    train_mae,
    "Train Set",
    "tabpfn Model Performance (Train Set)",
    train_path,
)
# 调用函数绘制测试集的拟合图
plot_regression_fit2(
    ...
    test_mae,
    "Test Set",
    "tabpfn Model Performance (Test Set)",
    test_path,
)
...
shap_values = interpretability.shap.get_shap_values(
    ...
    test_x=X_test_scaled_df,
    attribute_names=feature_names_from_df,
