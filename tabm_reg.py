...
import joblib
from tabm_model_tool import Tabm_model, tabm_config
...
# 解决 tabm 在 GridSearchCV 中可能出现的 verbosity 警告
...
config = tabm_config(
    ...
    n_num_features=X_train_scaled_df.shape[1],
...
best_model = Tabm_model(config)
if is_load == "n":
    best_model.fit(X_train_scaled_df.values, y_train.values)
    best_model.save_checkpoint(model_path)
else:
    best_model.load_checkpoint(path=model_path)
...
save_path_perm = os.path.join(
    results_plot_save_dir, "tabm_特征重要性_Permutation_final.png"
)  # 定义保存路径
# 调用函数绘制组合特征重要性图（使用置换重要性数据）
plot_importance_combined(
    perm_importance_df,
    "Permutation Importance for tabm",
    save_path_perm,
...
train_res_path = os.path.join(
    results_plot_save_dir, "tabm_训练集残差分析图_final.png"
)  # 训练集残差图保存路径
test_res_path = os.path.join(
    results_plot_save_dir, "tabm_验证集残差分析图_final.png"
)  # 测试集残差图保存路径
# 调用函数绘制训练集残差图
plot_residuals_styled(
    train_residuals,
    y_train_pred,
    train_res_path,
    "Train Set Residual Analysis of tabm",
)
# 调用函数绘制测试集残差图
plot_residuals_styled(
    test_residuals, y_test_pred, test_res_path, "Test Set Residual Analysis of tabm"
)

# =================================================================================
# ============ 使用手动计算方法绘制 PDP 和 ICE 相关图 (适用于tabm) ============
# =================================================================================
logger.info(
...
# 定义PDP/ICE图的保存目录
# --- 【tabm 修改】 ---
pdp_ice_save_dir = os.path.join(results_plot_save_dir, "tabm_PDP_ICE_Plots_final")
...
for feature_name in top_features_pdp_names:
    ...
    try:
        ...
        # --- 【tabm 修改】 ---
        plt.savefig(
            os.path.join(pdp_ice_save_dir, f"tabm_Manual_PDP_ICE_{feature_name}.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close(fig)  # 关闭图表
...
if len(top_features_pdp_names) >= 2:  # 确保至少有两个特征可以进行组合
    ...
    for feat1, feat2 in combinations(top_features_pdp_names, 2):
        ...
        try:
            ...
            plt.savefig(
                os.path.join(
                    pdp_2way_save_dir, f"tabm_Manual_PDP_2D_{feat1}_{feat2}.png"
                ),
                dpi=300,
            )  # 保存
            ...
            plt.savefig(
                os.path.join(
                    pdp_3d_save_dir, f"tabm_Manual_PDP_3D_{feat1}_{feat2}.png"
                ),
                dpi=300,
            )  # 保存
    ...
    for features_tuple in combinations(
        ...
        try:
            ...
            save_path = os.path.join(
                ...
                f"tabm_3D_Scatter_{features_tuple[0]}_{features_tuple[1]}_{features_tuple[2]}.png",
            )
            # 调用函数绘制3D散点图
            plot_3d_scatter_three_features(
        ...
        for feature_to_fix in features_tuple:
            try:
                ...
                save_path = os.path.join(
                    ...
                    f"tabm_3DPDP_{varying_feats[0]}_{varying_feats[1]}_Fix_{feature_to_fix}.png",
                )
                # 将固定的值设为该特征的中位数
...
# --- 【tabm 修改】 ---
logger.info("------------------------开始 SHAP 分析 (tabm)------------------------")
shap_save_dir = os.path.join(
    results_plot_save_dir, "tabm_SHAP_Plots_final"
)  # 定义SHAP图的保存目录
os.makedirs(shap_save_dir, exist_ok=True)  # 创建目录
# shap.TreeExplainer 同样适用于 tabm 模型
...
# 用pkl保存shap_values
with open(os.path.join(model_save_dir, "tabm_SHAP_values.pkl"), "wb") as f:
    pickle.dump(shap_values, f)

shap_values = pickle.load(
    open(os.path.join(model_save_dir, "tabm_SHAP_values.pkl"), "rb")
)
...
save_path_shap = os.path.join(
    shap_save_dir, "tabm_SHAP_特征重要性组合图_final.png"
)  # 定义保存路径
...
plt.savefig(
    os.path.join(shap_save_dir, "tabm_SHAP_summary_scatter.png"),
    dpi=300,
    bbox_inches="tight",
)  # 保存
plt.close()  # 关闭图表
...
for feature_name in top_features_pdp_names:
    ...
    plt.savefig(
        os.path.join(
            shap_dependence_save_dir, f"tabm_SHAP_dependence_{feature_name}.png"
        ),
        dpi=300,
        bbox_inches="tight",
    )  # 保存图表
...
plt.tight_layout()  # 调整布局
plt.savefig(
    os.path.join(shap_save_dir, "tabm_SHAP_waterfall_sample_0.png"),
    dpi=300,
    bbox_inches="tight",
)  # 保存
plt.close()  # 关闭图表
...
ale_save_dir = os.path.join(
    results_plot_save_dir, "tabm_ALE_Plots_final"
)  # 定义ALE图的保存目录
...
for i, feature_name in enumerate(top_features_ale_names):
    try:
        ...
        plt.tight_layout()  # 调整布局
        plt.savefig(
            os.path.join(ale_save_dir, f"tabm_ALE_1D_{feature_name}.png"),
            dpi=300,
            bbox_inches="tight",
        )  # 保存
        plt.close(fig)  # 关闭图表
...
if len(top_features_ale_names) >= 2:  # 确保至少有两个特征
    ...
    for feat1_name, feat2_name in combinations(top_features_ale_names, 2):
        try:
            ...
            plt.savefig(
                os.path.join(
                    ale_save_dir, f"tabm_ALE_2D_{feat1_name}_vs_{feat2_name}.png"
                ),
                dpi=300,
                bbox_inches="tight",
            )  # 保存图表
