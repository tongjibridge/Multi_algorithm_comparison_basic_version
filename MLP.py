...
# 解决 MLP 在 GridSearchCV 中可能出现的 verbosity 警告
...
def objective(trial):
    ...
    param_grid = {
        ...
        ),  # 隐藏层结构：(100,)表示1层100神经元；(100,50)表示2层
        "activation": "relu",  # 隐藏层激活函数
        "solver": "adam",  # 优化器（Adam自适应学习率）
        "max_iter": trial.suggest_int("max_iter", 300, 600),  # 最大迭代次数
        "verbose": False,  # 打印训练过程
...
model_path = os.path.join(
    model_save_dir, "MLP_model_final.pkl"
)  # 定义模型的完整保存路径
...
train_path = os.path.join(
    results_plot_save_dir, "MLP_训练集精度_final.png"
)  # 训练集拟合图的保存路径
...
plot_regression_fit(
    ...
    train_mae,
    "Train Set",
    "MLP Model Performance (Train Set)",
    train_path,
)
# 调用函数绘制测试集的拟合图
plot_regression_fit(
...
#     results_plot_save_dir, "MLP_特征重要性组合图_final.png"
# )  # 定义保存路径
# # 调用函数绘制组合特征重要性图
# plot_importance_combined(
#     gbdt_importance_df,
...
save_path_perm = os.path.join(
    results_plot_save_dir, "MLP_特征重要性_Permutation_final.png"
)  # 定义保存路径
# 调用函数绘制组合特征重要性图（使用置换重要性数据）
plot_importance_combined(
    perm_importance_df,
    "特征重要性 (Permutation Importance for MLP)",
    save_path_perm,
...
train_res_path = os.path.join(
    results_plot_save_dir, "MLP_训练集残差分析图_final.png"
)  # 训练集残差图保存路径
test_res_path = os.path.join(
    results_plot_save_dir, "MLP_验证集残差分析图_final.png"
)  # 测试集残差图保存路径
# 调用函数绘制训练集残差图
plot_residuals_styled(
    train_residuals, y_train_pred, train_res_path, "MLP 训练集残差分析"
)
# 调用函数绘制测试集残差图
plot_residuals_styled(test_residuals, y_test_pred, test_res_path, "MLP 验证集残差分析")

# # =================================================================================
# # ============ 使用手动计算方法绘制 PDP 和 ICE 相关图 (适用于MLP) ============
# # =================================================================================
# logger.info(
...
# # 定义PDP/ICE图的保存目录
# # --- 【MLP 修改】 ---
# pdp_ice_save_dir = os.path.join(results_plot_save_dir, "MLP_PDP_ICE_Plots_final")
...
#         # --- 【MLP 修改】 ---
#         plt.savefig(
#             os.path.join(pdp_ice_save_dir, f"MLP_Manual_PDP_ICE_{feature_name}.png"),
#             dpi=300,
#             bbox_inches="tight",
#         )
#         plt.close(fig)  # 关闭图表
...
#             plt.savefig(
#                 os.path.join(
#                     pdp_2way_save_dir, f"MLP_Manual_PDP_2D_{feat1}_{feat2}.png"
#                 ),
#                 dpi=300,
#             )  # 保存
...
#             # --- 【MLP 修改】 ---
#             plt.savefig(
#                 os.path.join(pdp_3d_save_dir, f"MLP_Manual_PDP_3D_{feat1}_{feat2}.png"),
#                 dpi=300,
#             )  # 保存
...
#                 f"MLP_3D_Scatter_{features_tuple[0]}_{features_tuple[1]}_{features_tuple[2]}.png",
#             )
#             # 调用函数绘制3D散点图
...
#                     f"MLP_3DPDP_{varying_feats[0]}_{varying_feats[1]}_Fix_{feature_to_fix}.png",
#                 )
#                 # 将固定的值设为该特征的中位数
...
# # --- 【MLP 修改】 ---
# logger.info("------------------------开始 SHAP 分析 (MLP)------------------------")
# shap_save_dir = os.path.join(
#     results_plot_save_dir, "MLP_SHAP_Plots_final"
# )  # 定义SHAP图的保存目录
# os.makedirs(shap_save_dir, exist_ok=True)  # 创建目录
# # shap.TreeExplainer 同样适用于 MLP 模型
...
#     shap_save_dir, "MLP_SHAP_特征重要性组合图_final.png"
# )  # 定义保存路径
...
# plt.savefig(
#     os.path.join(shap_save_dir, "MLP_SHAP_summary_scatter.png"),
#     dpi=300,
#     bbox_inches="tight",
# )  # 保存
# plt.close()  # 关闭图表
...
#     plt.savefig(
#         os.path.join(
#             shap_dependence_save_dir, f"MLP_SHAP_dependence_{feature_name}.png"
#         ),
#         dpi=300,
#         bbox_inches="tight",
#     )  # 保存图表
...
# plt.tight_layout()  # 调整布局
# plt.savefig(
#     os.path.join(shap_save_dir, "MLP_SHAP_waterfall_sample_0.png"),
#     dpi=300,
#     bbox_inches="tight",
# )  # 保存
# plt.close()  # 关闭图表
...
#     results_plot_save_dir, "MLP_ALE_Plots_final"
# )  # 定义ALE图的保存目录
...
#         plt.tight_layout()  # 调整布局
#         plt.savefig(
#             os.path.join(ale_save_dir, f"MLP_ALE_1D_{feature_name}.png"),
#             dpi=300,
#             bbox_inches="tight",
#         )  # 保存
#         plt.close(fig)  # 关闭图表
...
#             plt.savefig(
#                 os.path.join(
#                     ale_save_dir, f"MLP_ALE_2D_{feat1_name}_vs_{feat2_name}.png"
#                 ),
#                 dpi=300,
#                 bbox_inches="tight",
#             )  # 保存图表
