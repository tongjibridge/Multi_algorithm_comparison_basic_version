...
def build_model(device: torch.device, exponent, bandwidth, reg, verbose=True):
    rfm_params = {
        "model": {
            "kernel": "l2",
            "exponent": exponent,  # ↓ 降低非线性强度（更稳）
            "bandwidth": bandwidth,  # ↑ 让核更“平滑”（更强泛化）
            "diag": True,  # ✅ 更偏轴对齐/特征选择，表格数据更不容易过拟合
            "bandwidth_mode": "adaptive",  # 保留，但下面配合更强正则
        },
        "fit": {
            "reg": reg,  # ↑ 明显增大正则（最关键）
            "iters": 7,  # ↓ 少迭代一点，减少“学太细”
            "early_stop_rfm": True,  # ↓ 更早停（防止后期开始记噪声）
            "verbose": verbose,
        },
    }
    model = xRFM(
        ...
        task_type="regression",  # ⭐ 回归模式
        tuning_metric="mse",  # ⭐ 用 MSE 而不是 accuracy
...
def objective(trial):
    ...
    param_grid = {
        "exponent": trial.suggest_float("exponent", 1.0, 1.3, step=0.1),
        "bandwidth": trial.suggest_float("bandwidth", 4.0, 7.0, step=1.0),
        "reg": trial.suggest_float("reg", 3e-3, 7e-3, step=1e-3),
    }
    error = 0
    count = 0
    ...
    for train_index, test_index in kf.split(y):
        ...
        model = build_model(
            device=device,
            exponent=param_grid["exponent"],
            bandwidth=param_grid["bandwidth"],
            reg=param_grid["reg"],
            verbose=False,
        )
        model.fit(X_train_scaled_df, y_train, X_train_scaled_df, y_train)
...
# 解决 xrfm 在 GridSearchCV 中可能出现的 verbosity 警告
...
# study = optuna.create_study(direction="minimize", sampler=sampler)  # 最小化MSE
...
# print("最佳MSE:", study.best_value)


logger.info(
    "-------------------------------------训练模型---------------------------------------"
)
model_save_dir = r"./savemodel/xrfm2/"  # 定义模型保存的目录
os.makedirs(model_save_dir, exist_ok=True)  # 创建目录，如果目录已存在则不报错
# --- 【xrfm 修改】 ---
model_path = os.path.join(model_save_dir, "best.pt")  # 定义模型的完整保存路径
scaler_path = os.path.join(model_save_dir, "scaler.pkl")  # 定义scaler的完整保存路径
logger.debug(f"模型将保存至: {model_path}")  # 打印保存成功信息
# study_path = os.path.join(model_save_dir, "study.pkl")
...
# exponent = study.best_params["exponent"]
...
if is_load == "n":
    ...
    best_model.fit(X_train_scaled_df, y_train, X_train_scaled_df, y_train)
    ckpt = best_model.get_state_dict()
    torch.save(ckpt, model_path)
    # joblib保存X_train_scaled_df
    joblib.dump(
        X_train_scaled_df, os.path.join(model_save_dir, "X_train_scaled_df.pkl")
    )
else:
    best_model = build_model(device)
    ckpt = torch.load(model_path)
    # X_train_scaled_df = joblib.load(os.path.join(model_save_dir, "X_train_scaled_df.pkl"))
    X_train_scaled_tensor = torch.tensor(
        X_train_scaled_df, dtype=torch.float32, device=device
    )
    best_model.load_state_dict(ckpt, X_train_scaled_tensor)
...
results_plot_save_dir = r"./result/xrfm2/"  # 定义结果图保存的目录
...
train_path = os.path.join(
    results_plot_save_dir, "xrfm_训练集精度_final.png"
)  # 训练集拟合图的保存路径
test_path = os.path.join(
    results_plot_save_dir, "xrfm_验证集精度_final.png"
)  # 验证集拟合图的保存路径
# 调用函数绘制训练集的拟合图
plot_regression_fit2(
    y_train,
    ...
    train_mae,
    "Train Set",
    "xrfm Model Performance (Train Set)",
    train_path,
)
# 调用函数绘制测试集的拟合图
plot_regression_fit2(
    ...
    test_mae,
    "Test Set",
    "xrfm Model Performance (Test Set)",
    test_path,
)
...
for feat_name in feature_names_from_df:
    ...
    # 计算打乱后模型的R2分数
    shuff_acc = r2_score(y_test, best_model.predict(X_t.values))
...
save_path_perm = os.path.join(
    results_plot_save_dir, "xrfm_特征重要性_Permutation_final.png"
)  # 定义保存路径
# 调用函数绘制组合特征重要性图（使用置换重要性数据）
plot_importance_combined(
    perm_importance_df,
    "Permutation Importance for xrfm",
    save_path_perm,
...
train_res_path = os.path.join(
    results_plot_save_dir, "xrfm_训练集残差分析图_final.png"
)  # 训练集残差图保存路径
test_res_path = os.path.join(
    results_plot_save_dir, "xrfm_验证集残差分析图_final.png"
)  # 测试集残差图保存路径
# 调用函数绘制训练集残差图
plot_residuals_styled(
    train_residuals,
    y_train_pred,
    train_res_path,
    "Train Set Residual Analysis of xrfm",
)
# 调用函数绘制测试集残差图
plot_residuals_styled(
    test_residuals, y_test_pred, test_res_path, "Test Set Residual Analysis of xrfm"
)

# =================================================================================
# ============ 使用手动计算方法绘制 PDP 和 ICE 相关图 (适用于xrfm) ============
# =================================================================================
logger.info(
...
# 定义PDP/ICE图的保存目录
# --- 【xrfm 修改】 ---
pdp_ice_save_dir = os.path.join(results_plot_save_dir, "xrfm_PDP_ICE_Plots_final")
...
for feature_name in top_features_pdp_names:
    ...
    try:
        ...
        # --- 【xrfm 修改】 ---
        plt.savefig(
            os.path.join(pdp_ice_save_dir, f"xrfm_Manual_PDP_ICE_{feature_name}.png"),
            dpi=300,
            bbox_inches="tight",
        )
        plt.close(fig)  # 关闭图表
...
if len(top_features_pdp_names) >= 2:  # 确保至少有两个特征可以进行组合
    ...
    for feat1, feat2 in combinations(top_features_pdp_names, 2):
        ...
        try:
            ...
            plt.savefig(
                os.path.join(
                    pdp_2way_save_dir, f"xrfm_Manual_PDP_2D_{feat1}_{feat2}.png"
                ),
                dpi=300,
            )  # 保存
            ...
            plt.savefig(
                os.path.join(
                    pdp_3d_save_dir, f"xrfm_Manual_PDP_3D_{feat1}_{feat2}.png"
                ),
                dpi=300,
            )  # 保存
    ...
    for features_tuple in combinations(
        ...
        try:
            ...
            save_path = os.path.join(
                ...
                f"xrfm_3D_Scatter_{features_tuple[0]}_{features_tuple[1]}_{features_tuple[2]}.png",
            )
            # 调用函数绘制3D散点图
            plot_3d_scatter_three_features(
        ...
        for feature_to_fix in features_tuple:
            try:
                ...
                save_path = os.path.join(
                    ...
                    f"xrfm_3DPDP_{varying_feats[0]}_{varying_feats[1]}_Fix_{feature_to_fix}.png",
                )
                # 将固定的值设为该特征的中位数
...
# --- 【xrfm 修改】 ---
logger.info("------------------------开始 SHAP 分析 (xrfm)------------------------")
shap_save_dir = os.path.join(
    results_plot_save_dir, "xrfm_SHAP_Plots_final"
)  # 定义SHAP图的保存目录
os.makedirs(shap_save_dir, exist_ok=True)  # 创建目录
# shap.TreeExplainer 同样适用于 xrfm 模型
# explainer = shap.TreeExplainer(best_model)
# shap_values = explainer(X_test_scaled_df)  # 计算测试集所有样本的SHAP值

explainer = shap.Explainer(best_model.predict, X_test_scaled_df)
shap_values = explainer(X_test_scaled_df)
# 用pkl保存shap_values
with open(os.path.join(model_save_dir, "xrfm_SHAP_values.pkl"), "wb") as f:
    pickle.dump(shap_values, f)

shap_values = pickle.load(
    open(os.path.join(model_save_dir, "xrfm_SHAP_values.pkl"), "rb")
)
...
save_path_shap = os.path.join(
    shap_save_dir, "xrfm_SHAP_特征重要性组合图_final.png"
)  # 定义保存路径
...
plt.savefig(
    os.path.join(shap_save_dir, "xrfm_SHAP_summary_scatter.png"),
    dpi=300,
    bbox_inches="tight",
)  # 保存
plt.close()  # 关闭图表
...
for feature_name in top_features_pdp_names:
    ...
    plt.savefig(
        os.path.join(
            shap_dependence_save_dir, f"xrfm_SHAP_dependence_{feature_name}.png"
        ),
        dpi=300,
        bbox_inches="tight",
    )  # 保存图表
...
plt.tight_layout()  # 调整布局
plt.savefig(
    os.path.join(shap_save_dir, "xrfm_SHAP_waterfall_sample_0.png"),
    dpi=300,
    bbox_inches="tight",
)  # 保存
plt.close()  # 关闭图表
...
ale_save_dir = os.path.join(
    results_plot_save_dir, "xrfm_ALE_Plots_final"
)  # 定义ALE图的保存目录
...
for i, feature_name in enumerate(top_features_ale_names):
    try:
        ...
        plt.tight_layout()  # 调整布局
        plt.savefig(
            os.path.join(ale_save_dir, f"xrfm_ALE_1D_{feature_name}.png"),
            dpi=300,
            bbox_inches="tight",
        )  # 保存
        plt.close(fig)  # 关闭图表
...
if len(top_features_ale_names) >= 2:  # 确保至少有两个特征
    ...
    for feat1_name, feat2_name in combinations(top_features_ale_names, 2):
        try:
            ...
            plt.savefig(
                os.path.join(
                    ale_save_dir, f"xrfm_ALE_2D_{feat1_name}_vs_{feat2_name}.png"
                ),
                dpi=300,
                bbox_inches="tight",
            )  # 保存图表
