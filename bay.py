...
# 解决 BayesianRidge 在 GridSearchCV 中可能出现的 verbosity 警告
...
def objective(trial):
    global index
    param_grid = {
        "alpha_1": trial.suggest_float("alpha_1", 1e-6, 1e-3, log=True),  # 正则化参数1
        "alpha_2": trial.suggest_float("alpha_2", 1e-6, 1e-3, log=True),  # 正则化参数2
        "lambda_1": trial.suggest_float(
            "lambda_1", 1e-10, 1e-4, log=True
        ),  # 正则化参数3
        "lambda_2": trial.suggest_float(
            "lambda_2", 1e-10, 1e-4, log=True
        ),  # 正则化参数4
        "max_iter": trial.suggest_int("max_iter", 200, 400),  # 最大迭代次数
    }
...
# --- 【BayesianRidge 修改】 ---
logger.info(
    "-------------------------------------定义BayesianRidge模型超参数范围---------------------------------------"
)
# 定义要进行网格搜索的BayesianRidge超参数范围
...
model_save_dir = r"./savemodel/BayesianRidge/"  # 定义模型保存的目录
os.makedirs(model_save_dir, exist_ok=True)  # 创建目录，如果目录已存在则不报错
# --- 【BayesianRidge 修改】 ---
model_path = os.path.join(
    model_save_dir, "BayesianRidge_model_final.pkl"
)  # 定义模型的完整保存路径
...
best_model = BayesianRidge(**study.best_params)
...
results_plot_save_dir = r"./result/BayesianRidge/"  # 定义结果图保存的目录
...
with pd.ExcelWriter(
    os.path.join(results_plot_save_dir, "BayesianRidge_scaled_results.xlsx")
) as writer:
...
test_df.columns = feature_names_from_df + ["y_test", "y_test_pred"]
with pd.ExcelWriter(
    os.path.join(results_plot_save_dir, "BayesianRidge_results.xlsx")
) as writer:
    train_df.to_excel(writer, sheet_name="train", index=False)
    test_df.to_excel(writer, sheet_name="test", index=False)
...
train_path = os.path.join(
    results_plot_save_dir, "BayesianRidge_训练集精度_final.png"
)  # 训练集拟合图的保存路径
test_path = os.path.join(
    results_plot_save_dir, "BayesianRidge_验证集精度_final.png"
)  # 验证集拟合图的保存路径
# 调用函数绘制训练集的拟合图
plot_regression_fit(
    y_train,
    ...
    train_mae,
    "Train Set",
    "BayesianRidge Model Performance (Train Set)",
    train_path,
)
# 调用函数绘制测试集的拟合图
plot_regression_fit(
    ...
    test_mae,
    "Test Set",
    "BayesianRidge Model Performance (Test Set)",
    test_path,
)
...
#     results_plot_save_dir, "BayesianRidge_特征重要性组合图_final.png"
# )  # 定义保存路径
# # 调用函数绘制组合特征重要性图
# plot_importance_combined(
#     gbdt_importance_df,
#     "BayesianRidge模型计算的特征重要性",
#     save_path_gbdt,
...
save_path_perm = os.path.join(
    results_plot_save_dir, "BayesianRidge_特征重要性_Permutation_final.png"
)  # 定义保存路径
# 调用函数绘制组合特征重要性图（使用置换重要性数据）
plot_importance_combined(
    perm_importance_df,
    "特征重要性 (Permutation Importance for BayesianRidge)",
    save_path_perm,
...
train_res_path = os.path.join(
    results_plot_save_dir, "BayesianRidge_训练集残差分析图_final.png"
)  # 训练集残差图保存路径
test_res_path = os.path.join(
    results_plot_save_dir, "BayesianRidge_验证集残差分析图_final.png"
)  # 测试集残差图保存路径
# 调用函数绘制训练集残差图
plot_residuals_styled(
    train_residuals, y_train_pred, train_res_path, "BayesianRidge 训练集残差分析"
)
# 调用函数绘制测试集残差图
plot_residuals_styled(
    test_residuals, y_test_pred, test_res_path, "BayesianRidge 验证集残差分析"
)

# # =================================================================================
# # ============ 使用手动计算方法绘制 PDP 和 ICE 相关图 (适用于BayesianRidge) ============
# # =================================================================================
# logger.info(
...
# # 定义PDP/ICE图的保存目录
# # --- 【BayesianRidge 修改】 ---
# pdp_ice_save_dir = os.path.join(results_plot_save_dir, "BayesianRidge_PDP_ICE_Plots_final")
...
# # 根据BayesianRidge置换重要性排序，选取最重要的N个特征
...
#         # --- 【BayesianRidge 修改】 ---
#         plt.savefig(
#             os.path.join(pdp_ice_save_dir, f"BayesianRidge_Manual_PDP_ICE_{feature_name}.png"),
#             dpi=300,
#             bbox_inches="tight",
#         )
#         plt.close(fig)  # 关闭图表
...
#             plt.savefig(
#                 os.path.join(
#                     pdp_2way_save_dir, f"BayesianRidge_Manual_PDP_2D_{feat1}_{feat2}.png"
#                 ),
#                 dpi=300,
#             )  # 保存
...
#             # --- 【BayesianRidge 修改】 ---
#             plt.savefig(
#                 os.path.join(pdp_3d_save_dir, f"BayesianRidge_Manual_PDP_3D_{feat1}_{feat2}.png"),
#                 dpi=300,
#             )  # 保存
...
#                 f"BayesianRidge_3D_Scatter_{features_tuple[0]}_{features_tuple[1]}_{features_tuple[2]}.png",
#             )
#             # 调用函数绘制3D散点图
...
#                     f"BayesianRidge_3DPDP_{varying_feats[0]}_{varying_feats[1]}_Fix_{feature_to_fix}.png",
#                 )
#                 # 将固定的值设为该特征的中位数
...
# # --- 【BayesianRidge 修改】 ---
# logger.info("------------------------开始 SHAP 分析 (BayesianRidge)------------------------")
# shap_save_dir = os.path.join(
#     results_plot_save_dir, "BayesianRidge_SHAP_Plots_final"
# )  # 定义SHAP图的保存目录
# os.makedirs(shap_save_dir, exist_ok=True)  # 创建目录
# # shap.TreeExplainer 同样适用于 BayesianRidge 模型
...
#     shap_save_dir, "BayesianRidge_SHAP_特征重要性组合图_final.png"
# )  # 定义保存路径
...
# plt.savefig(
#     os.path.join(shap_save_dir, "BayesianRidge_SHAP_summary_scatter.png"),
#     dpi=300,
#     bbox_inches="tight",
# )  # 保存
# plt.close()  # 关闭图表
...
#     plt.savefig(
#         os.path.join(
#             shap_dependence_save_dir, f"BayesianRidge_SHAP_dependence_{feature_name}.png"
#         ),
#         dpi=300,
#         bbox_inches="tight",
#     )  # 保存图表
...
# plt.tight_layout()  # 调整布局
# plt.savefig(
#     os.path.join(shap_save_dir, "BayesianRidge_SHAP_waterfall_sample_0.png"),
#     dpi=300,
#     bbox_inches="tight",
# )  # 保存
# plt.close()  # 关闭图表
...
# # --- 【BayesianRidge 修改】 ---
# logger.info(
#     "----------------------------------------开始 ALE 分析 (BayesianRidge)-----------------------------------------"
# )
# ale_save_dir = os.path.join(
#     results_plot_save_dir, "BayesianRidge_ALE_Plots_final"
# )  # 定义ALE图的保存目录
...
#         plt.tight_layout()  # 调整布局
#         plt.savefig(
#             os.path.join(ale_save_dir, f"BayesianRidge_ALE_1D_{feature_name}.png"),
#             dpi=300,
#             bbox_inches="tight",
#         )  # 保存
#         plt.close(fig)  # 关闭图表
...
#             plt.savefig(
#                 os.path.join(
#                     ale_save_dir, f"BayesianRidge_ALE_2D_{feat1_name}_vs_{feat2_name}.png"
#                 ),
#                 dpi=300,
#                 bbox_inches="tight",
#             )  # 保存图表
