...
# 解决 GBR 在 GridSearchCV 中可能出现的 verbosity 警告
...
def objective(trial):
    ...
    param_grid = {
        ...
        "n_estimators": trial.suggest_int(
            "n_estimators", 50, 300
        ),  # 正则化参数,0.001, 0.01, 0.1, 1, 10
        "max_depth": trial.suggest_int("max_depth", 3, 14),  # 核函数类型
        ...
        "alpha": 0.5,  # 正则化参数,0.001, 0.01, 0.1, 1, 10
        # 'epsilon': [0.1]  # epsilon-GBR模型中的epsilon, 0.2, 0.5
    }
...
model_save_dir = r"./savemodel/GBR/"  # 定义模型保存的目录
os.makedirs(model_save_dir, exist_ok=True)  # 创建目录，如果目录已存在则不报错
model_path = os.path.join(
    model_save_dir, "GBR_model_final.pkl"
)  # 定义模型的完整保存路径
...
results_plot_save_dir = r"./result/GBR/"  # 定义结果图保存的目录
...
with pd.ExcelWriter(
    os.path.join(results_plot_save_dir, "GBR_scaled_results.xlsx")
) as writer:
...
test_df.columns = feature_names_from_df + ["y_test", "y_test_pred"]
with pd.ExcelWriter(os.path.join(results_plot_save_dir, "GBR_results.xlsx")) as writer:
    train_df.to_excel(writer, sheet_name="train", index=False)
    test_df.to_excel(writer, sheet_name="test", index=False)
...
train_path = os.path.join(
    results_plot_save_dir, "GBR_训练集精度_final.png"
)  # 训练集拟合图的保存路径
test_path = os.path.join(
    results_plot_save_dir, "GBR_验证集精度_final.png"
)  # 验证集拟合图的保存路径
# 调用函数绘制训练集的拟合图
plot_regression_fit(
    y_train,
    ...
    train_mae,
    "Train Set",
    "GBR Model Performance (Train Set)",
    train_path,
)
# 调用函数绘制测试集的拟合图
plot_regression_fit(
    ...
    test_mae,
    "Test Set",
    "GBR Model Performance (Test Set)",
    test_path,
)
...
#     results_plot_save_dir, "GBR_特征重要性组合图_final.png"
# )  # 定义保存路径
# # 调用函数绘制组合特征重要性图
# plot_importance_combined(
#     gbdt_importance_df,
...
save_path_perm = os.path.join(
    results_plot_save_dir, "GBR_特征重要性_Permutation_final.png"
)  # 定义保存路径
# 调用函数绘制组合特征重要性图（使用置换重要性数据）
plot_importance_combined(
    perm_importance_df,
    "特征重要性 (Permutation Importance for GBR)",
    save_path_perm,
...
train_res_path = os.path.join(
    results_plot_save_dir, "GBR_训练集残差分析图_final.png"
)  # 训练集残差图保存路径
test_res_path = os.path.join(
    results_plot_save_dir, "GBR_验证集残差分析图_final.png"
)  # 测试集残差图保存路径
# 调用函数绘制训练集残差图
plot_residuals_styled(
    train_residuals, y_train_pred, train_res_path, "GBR 训练集残差分析"
)
# 调用函数绘制测试集残差图
plot_residuals_styled(test_residuals, y_test_pred, test_res_path, "GBR 验证集残差分析")

# # =================================================================================
# # ============ 使用手动计算方法绘制 PDP 和 ICE 相关图 (适用于GBR) ============
# # =================================================================================
# logger.info(
...
# # 定义PDP/ICE图的保存目录
# # --- 【GBR 修改】 ---
# pdp_ice_save_dir = os.path.join(results_plot_save_dir, "GBR_PDP_ICE_Plots_final")
...
#         # --- 【GBR 修改】 ---
#         plt.savefig(
#             os.path.join(pdp_ice_save_dir, f"GBR_Manual_PDP_ICE_{feature_name}.png"),
#             dpi=300,
#             bbox_inches="tight",
#         )
#         plt.close(fig)  # 关闭图表
...
#         logger.info(f"正在计算特征对 '{feat1}' vs '{feat2}' 的2D PDP数据...")
#         try:
#             # 使用手动编写的函数计算2D PDP数据
...
#             plt.savefig(
#                 os.path.join(
#                     pdp_2way_save_dir, f"GBR_Manual_PDP_2D_{feat1}_{feat2}.png"
#                 ),
#                 dpi=300,
#             )  # 保存
...
#             # --- 【GBR 修改】 ---
#             plt.savefig(
#                 os.path.join(pdp_3d_save_dir, f"GBR_Manual_PDP_3D_{feat1}_{feat2}.png"),
#                 dpi=300,
#             )  # 保存
...
#                 f"GBR_3D_Scatter_{features_tuple[0]}_{features_tuple[1]}_{features_tuple[2]}.png",
#             )
#             # 调用函数绘制3D散点图
...
#                     f"GBR_3DPDP_{varying_feats[0]}_{varying_feats[1]}_Fix_{feature_to_fix}.png",
#                 )
#                 # 将固定的值设为该特征的中位数
...
# # --- 【GBR 修改】 ---
# logger.info("------------------------开始 SHAP 分析 (GBR)------------------------")
# shap_save_dir = os.path.join(
#     results_plot_save_dir, "GBR_SHAP_Plots_final"
# )  # 定义SHAP图的保存目录
# os.makedirs(shap_save_dir, exist_ok=True)  # 创建目录
# X_train_summary = shap.kmeans(X_train_scaled_df, 100)
...
#     shap_save_dir, "GBR_SHAP_特征重要性组合图_final.png"
# )  # 定义保存路径
...
# plt.savefig(
#     os.path.join(shap_save_dir, "GBR_SHAP_summary_scatter.png"),
#     dpi=300,
#     bbox_inches="tight",
# )  # 保存
# plt.close()  # 关闭图表
...
#     plt.savefig(
#         os.path.join(
#             shap_dependence_save_dir, f"GBR_SHAP_dependence_{feature_name}.png"
#         ),
#         dpi=300,
#         bbox_inches="tight",
#     )  # 保存图表
...
# plt.tight_layout()  # 调整布局
# plt.savefig(
#     os.path.join(shap_save_dir, "GBR_SHAP_waterfall_sample_0.png"),
#     dpi=300,
#     bbox_inches="tight",
# )  # 保存
# plt.close()  # 关闭图表
...
#     results_plot_save_dir, "GBR_ALE_Plots_final"
# )  # 定义ALE图的保存目录
...
#         plt.tight_layout()  # 调整布局
#         plt.savefig(
#             os.path.join(ale_save_dir, f"GBR_ALE_1D_{feature_name}.png"),
#             dpi=300,
#             bbox_inches="tight",
#         )  # 保存
#         plt.close(fig)  # 关闭图表
...
#             plt.savefig(
#                 os.path.join(
#                     ale_save_dir, f"GBR_ALE_2D_{feat1_name}_vs_{feat2_name}.png"
#                 ),
#                 dpi=300,
#                 bbox_inches="tight",
#             )  # 保存图表
