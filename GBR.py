...
model_save_dir = r"./savemodel/GBR2/"  # 定义模型保存的目录
...
results_plot_save_dir = r"./result/GBR2/"  # 定义结果图保存的目录
...
with pd.ExcelWriter(
    os.path.join(results_plot_save_dir, "scaled_results.xlsx")
) as writer:
...
test_df.columns = feature_names_from_df + ["y_test", "y_test_pred"]
with pd.ExcelWriter(os.path.join(results_plot_save_dir, "results.xlsx")) as writer:
    train_df.to_excel(writer, sheet_name="train", index=False)
    test_df.to_excel(writer, sheet_name="test", index=False)
